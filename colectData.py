...
def colectData():
    ...
    indici_sapt = (indici_sapt - data_ini).astype("timedelta64[D]").astype(
        np.int64
    ) // 7
...
def queryData():
    ...
    result = result.drop(["result", "table", "_start", "_stop"])

    result = result.rename({"_time": "ds", "_measurement": "unique_id", "Close": "y"})
...
def deleteAllData():
    ...
    delete_api.delete(start, stop, '_measurement="stock"', bucket=database, org=org)
